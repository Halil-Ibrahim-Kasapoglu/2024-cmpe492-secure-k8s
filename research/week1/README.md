...
- We have also provided a Python script [plotter.py](plotter.py) to analyze the results and generate plots. The script reads the output files generated by the ab tests and generates plots for different metrics such as Requests per second, Time per request, and Failed requests. To run the script make sure you have the necessary Python libraries installed
